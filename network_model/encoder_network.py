...
class EncoderNetwork(BaseNetwork):
    ...
    def __init__(self, number_of_neurons_per_layer, max_number_of_leaves):
    ...
    def encode_dataset(self):
        ...
        encoded_dataset = tf.reshape(encoded_dataset, [tf.shape(self.dna_sequence_node_1)[0], self.max_number_of_leaves *
                                                       self.number_of_neurons_per_layer[-1]])

        encoder_2_w, encoder_2_b = [], []

        tensorflow_utils.create_and_append_matrix(
            self.max_number_of_leaves * self.number_of_neurons_per_layer[-1],
